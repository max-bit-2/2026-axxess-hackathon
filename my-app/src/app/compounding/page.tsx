  ...
  return (
    <AppShell userLabel={String(displayName)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Compounding Overview</h1>
          <p className="text-slate-500 mt-1">Real-time compounding workflow and verification queue.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center rounded-lg border border-slate-200 bg-white p-1">
            <Link
              href={q ? `/compounding?q=${encodeURIComponent(q)}` : "/compounding"}
            ...
            <Link
              href={
                q
                  ? `/compounding?view=all&q=${encodeURIComponent(q)}`
            ...
            href={
              activeView === "all"
                ? q
                  ...
                  : "/compounding?view=all"
                : q
                  ? `/compounding?q=${encodeURIComponent(q)}`
                  : "/compounding"
            }
              ...
              {queue.map((item) => (
                <tr key={item.jobId} className="group hover:bg-blue-50/30 :bg-blue-900/10 transition-colors">
                  ...
                  <td className="px-6 py-3 text-right">
                    <Link href={`/compounding/jobs/${item.jobId}`}>
